...
const char* recognize(uint8_t* buffer, size_t size, int fallback)
{
    ...
    cout << "Decode cost: " << (end - start) / getTickFrequency() * 1000 << endl;
    Analyzer analyzer;
    AnalyzeResult result;

    try {
        start = getTickCount();
        result = analyzer.analyze(img, (bool)fallback);
        end = getTickCount();
